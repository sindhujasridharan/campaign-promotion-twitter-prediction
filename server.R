...
api_key <- "twiter api key"
api_secret <- "twiter api secret"
token <- "twiter api token" 
token_secret <- "twiter api token secret" 
...
shinyServer(
  function(input, output, session) {
    ...
    observeEvent(input$timeSeries, { 
      ...
      tweetScore.ts<-ts(forecastScores$x,start=c(20164100),freq=24)
    ...
    observeEvent(input$simpleForecast, { 
      forecastScores <- read.csv(file=paste('datafiles/',selectedData(), '_forecastScores.csv'))
      tweetScore.ts<-ts(forecastScores$x,start=c(20164100),freq=8640)
